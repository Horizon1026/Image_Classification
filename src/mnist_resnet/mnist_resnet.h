...
public:
    ...
    struct ResNetBlock: torch::nn::Module {
        ResNetBlock(int32_t in_planes, int32_t planes, int32_t stride = 1):
            conv1(torch::nn::Conv2dOptions(in_planes, planes, 3).stride(stride).padding(1).bias(false)), bn1(planes),
            conv2(torch::nn::Conv2dOptions(planes, planes, 3).stride(1).padding(1).bias(false)), bn2(planes) {
    ...
    struct ResNet: torch::nn::Module {
        ResNet(int32_t num_classes = 10, int32_t init_channels = 16): fc(init_channels * 4, num_classes) {
